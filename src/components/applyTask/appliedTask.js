...
export default function AppliedTask(props) {
  ...
  useEffect(() => {
    if (props.state != "") {
      axios
        .get(`http://localhost:8181/application/unstartedJobSearch/1/${size}/${name}/${props.state}`)
        .then(function (resp) {
          setResult(resp.data);
        });
      axios
        .get(`http://192.168.0.6:8181/application/totalUnstartedJobSearch/${name}/${props.state}`)
        .then((res) => {
  ...
  useEffect(() => {
    axios.get(`http://192.168.0.6:8181/application/unstartedJob/1/${size}/${name}`)
    .then(res => {
      console.log(res)
      setResult(res.data);
      setTemData(res.data);
    })
    axios.get(`http://192.168.0.6:8181/application/totalUnstartedJob/${name}`).then((res) => {
    setTotalRes(res.data);
    ...
    //   </Button>}
      ...
      <Pagination
        ...
        onChange={(e) => {
          if (props.state == "") {
            axios
            .get(`http://192.168.0.6:8181/application/unstartedJob/${e}/${size}/${name}`)
            .then((res) => {
