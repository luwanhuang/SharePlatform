...
export default function OngoingTask(props) {
  ...
  useEffect(() => {
    if (props.state != "") {
      axios
        .get(`http://localhost:8181/ongoingTask/ongoingSearch/1/${size}/${name}/${props.state}`)
        .then(function (resp) {
          setResult(resp.data);
        });
      axios
        .get(`http://192.168.0.6:8181/ongoingTask/ongoingSearchTotal/${name}/${props.state}`)
        .then((res) => {
  ...
  useEffect(() => {
    axios.get(`http://192.168.0.6:8181/ongoingTask/ongoing/1/${size}/${name}`)
    .then(res => {
      console.log(res)
      setResult(res.data);
      setTemData(res.data);
    })
    axios.get(`http://192.168.0.6:8181/ongoingTask/ongoingTotal/${name}`).then((res) => {
    setTotalRes(res.data);
    ...
    <div className = "outFormD">
      ...
      <Pagination
        ...
        onChange={(e) => {
          if (props.state == "") {
            axios
            .get(`http://192.168.0.6:8181/ongoingTask/ongoing/${e}/${size}/${name}`)
            .then((res) => {
